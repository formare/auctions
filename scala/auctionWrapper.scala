...
// Anything after the line "object Rextester extends App {" and this line has been generated by Isabelle. 
...
def printAllocation(args: (BigInt, List[BigInt])): Unit = args match {
...
def printAllocations(args: set[List[(BigInt, List[BigInt])]]): Unit = {
    choice(args).foreach(printAllocation);
  }
...
println;
println("Warning: We just print the first allocation. Choice must be replaced by the_elem");
println("Maximal allocation:"); 
printAllocations(example);
};
