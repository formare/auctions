...
definition weakefficient where "weakefficient \<FF>   i \<ff> v a1 a2 \<eta>= 
(\<exists> f1 f2::nat => valuation.(  
((\<eta>(f1))(a2)) - (\<eta>(f1)(a1)) ----> v
&
((\<eta>(f2))(a2)) - (\<eta>(f2)(a1)) ----> v
&
((%j. (\<FF>``{\<ff>+(i, f1 j)}))=(%j. {a1})) & 
((%j. (\<FF>``{\<ff>+(i,f2 j)}))=(%j. {a2}))))"
...
The functions in_rel and rel_in convert between those two worlds. *)
assumes 
"effic 0 1 A"
"i \<in> Domain b"
"runiq b"
