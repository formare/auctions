...
where "revenue_rel b buyer  = (let Y = Domain buyer in
  \<Sum> y \<in> Y . b (eval_rel buyer y) y
...
value "hd (* trivial tie-breaking: take the first allocation *)
  (arg_max_comp_list
    (possible_allocations_comp {65,66::nat} {1,2,3::nat})
    (revenue_rel (\<lambda> bidder goods . 
      if (bidder = 1 \<and> goods = {65,66}
