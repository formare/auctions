...
proof -
  ...
  {
    fix b::bid fix Y let ?bv="b+*({i}\<times>Y)" let ?v="the_elem Y" 
    let ?y="%x. ?v x" assume 
    ...
    2: "Y={?v}" using 1 by (metis subset_singletonD trivial_def) then 
    have 
    "?v (a,,b)-(p,,b)\<le>?v (a,,(b+*({i}\<times>{?v})))-(p,,(b+*({i}\<times>{?v})))" 
...
shows "(p::(bid \<times> price) set),,b = 
((projector ((reducedbid i a)^-1)) O (quotient p (quotientbid i a) Id) O ((projector Id)^-1))
,, (reducedbid i (a::(bid \<times> allocation) set),,b)"
proof -  
  term "a"
