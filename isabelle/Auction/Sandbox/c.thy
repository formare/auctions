...
proof -
  (* TODO CL: This step was generated by Sledgehammer and needs cleaning for readability. *)
  have "\<And>u x\<^sub>1 b_x. runiq u \<and> (x\<^sub>1\<Colon>'b) \<in> u `` {b_x\<Colon>'a} \<longrightarrow> x\<^sub>1 = u ,, b_x"
    using runiq_wrt_eval_rel by fastforce
  then show "y = f ,, x"
...
(*  hence "\<forall> y2 \<in> R``{x}. y2\<in>{y}" using assms runiq_alt runiq_def ll1 by blast *)
  hence "R `` {x} \<subseteq> {y}" using assms runiq_def runiq_alt by blast
  hence "trivial (R``{x})" using trivial_def assms 0 by (metis order_refl the_elem_eq trivial_subset)
}
thus ?thesis using runiq_alt trivial_def by blast
qed
...
using assms runiq_alt trivial_def compatible_def by 
...
{
  fix x have 0: "?t (f``{x})" using assms by (metis runiq_alt) 
...
lemma assumes "finite X" shows "trivial X=(card X \<le> 1)" (is "?LH=?RH") using trivial_def assms 
proof -
  {
    assume "card X=1" 
    hence "X = {the_elem X}" 
...
by auto
moreover have 
3: "\<forall>x y. {y} \<subseteq> f``{x} \<longrightarrow> {y}=f``{x}" using assms runiq_def trivial_singleton ll11 by metis
ultimately have 
