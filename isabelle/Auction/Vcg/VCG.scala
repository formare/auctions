...
def goodsList[A, B : equal : linorder, C](b: List[(A, (List[B], C))]): List[B] =
  sorted_list_of_set[B](SSup_set[B](image[(A, (List[B], C)),
...
def printPrice(b: List[(BigInt, (List[BigInt], BigInt))], r: BigInt, participant: BigInt, alloc: set[(BigInt, set[BigInt])]) {
    println(payments(b, r, participant, alloc));
}
...
def printAllocationAndPayment(args: (BigInt, List[BigInt]), b: List[(BigInt, (List[BigInt], BigInt))], r: BigInt, alloc: set[(BigInt, set[BigInt])]):
    Unit = args match {
               case (hd, tl) => print(" X_" + hd + " = {" ); 
                                printListOfGoods(tl);
                                print("}    payment:");
                                printPrice(b, r, hd, alloc);
  }


// The Isabelle generated code will compute an allocation in form of list of pairs (participant, listOfWonItems).
// printAllocationAndPayments prints the allocation and the price to be paid for each such pair one by one.
def printAllocationsAndPayments(args: set[List[(BigInt, List[BigInt])]], b: List[(BigInt, (List[BigInt], BigInt))], r: BigInt, alloc: set[(BigInt, set[BigInt])]):
   Unit = { choice(args).foreach(arg => printAllocationAndPayment(arg, b, r, alloc));
  }
...
def runExample(b: List[(BigInt, (List[BigInt], BigInt))], r: BigInt):
   Unit = {
      ...
      var vcga = vcgaAlg(participantsSet(b), goodsList(b), Bid2funcBid(b) , r);
      printAllocationsAndPayments(allocationPrettyPrint(vcga), b, r, vcga);
      println("************************************************************************************************");
}
...
// START OF EXAMPLE e4 with bids b4 and random number r4

val r4: BigInt = 0

val b4: List[(BigInt, (List[BigInt], BigInt))] =
    List((1, (List(11, 12, 13, 14), 20)),
         (1, (List(11, 12, 14), 18)),
         (2, (List(11), 10)),
         (2, (List(12), 15)),
         (2, (List(13), 15)),
...
// START OF EXAMPLE e5 with bids b5 and random number r5
val r5: BigInt = 0

val b5: List[(BigInt, (List[BigInt], BigInt))] =
    List((1, (List(11, 12, 13, 14, 15), 20)),
         (1, (List(11, 12, 14, 15), 18)),
         (2, (List(11), 10)),
         (2, (List(12), 15)),
         (2, (List(13), 15)),
         (2, (List(12,13), 18)),
