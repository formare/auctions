...
proof -
  ...
  let ?b1="%j. b+*(?I \<times> {v1 j})" let ?b2="%j. b+*(?I \<times> {v2 j})"
  {
    ...
    "f``{?b1 j}=F``{?b1 j} & f``{?b2 j}=F``{?b2 j}" using 1 2 SetUtils.ll11 by metis
...
proof -
  ...
  1: "v1 ----> ?M & (\<forall> j.(v1 j < ?M)) & v2 ----> ?M & (\<forall> j.(v2 j > ?M))" using ll29 by blast
  {
    ...
    ultimately have "Sup (?r ?b2)=max (v2 j) ?M" 
    using 0 cSup_insert sup_real_def by (metis bdd_aboveI)
    (* by (metis (lifting) cSup_insert sup_real_def) *)
  ...
  Range (b outside {i}) \<union> {v2 j}` `Sup (Range (b +* {i} \<times> {v2 j})) = max (v2 j) (Sup (Range (b outside {i})))` insert_absorb insert_def less_irrefl)
    ...
    moreover have "... = ?GGG``{?b2}" unfolding Image_def by blast
ultimately have 
  ...
  *)
    ...
    ultimately have "Sup (?r ?b1)=max (v1 j) ?M" 
    using 0 cSup_insert sup_real_def bdd_aboveI by (metis (lifting)) 
    ...
    by (metis Image_runiq_eq_eval assms(2) assms(4))  
    moreover have "... = ?GGG``{?b1}" unfolding Image_def by blast ultimately have 
...
proof -
  ...
  moreover have "...={?af b}" using assms runiqs_def 1 unfolding Image_def by blast
  ...
  moreover have "...={?pf b}" using assms runiqs_def 1 unfolding Image_def by blast
